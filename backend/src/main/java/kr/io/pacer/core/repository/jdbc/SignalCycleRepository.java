    ...
    public Map<Integer, Map<String, SignalCycle>> findByItstIds(List<Integer> itstIds) {
        ...
        String sql = """
                SELECT itst_id, direction,
                       CAST(NULLIF(ROUND(CAST(SUM(CASE WHEN state != 'stop-And-Remain' THEN cycle_seconds ELSE 0 END) AS numeric), 2), 0) AS double precision) AS green_avg_sec,
                       CAST(ROUND(CAST(MAX(CASE WHEN state = 'stop-And-Remain' THEN cycle_seconds END) AS numeric), 2) AS double precision) AS red_avg_sec
